...
<template>
  <div>
    <div class="bg-base-200 px-4 pt-3 pb-4 rounded-lg shadow">
      <div class="flex flex-col lg:!flex-row pt-2 pb-1">
        <div class="flex-1">
          ...
          <div class="m-4 text-sm">
            <p class="text-sm mb-3 font-bold">{{ $t('staking.about_us') }}</p>
            <div class="card-list">
              <div class="flex items-center mb-2">
                <Icon icon="mdi-web" class="text-xl mr-1" />
                <span class="font-medium mr-2">{{ $t('staking.website') }}: </span>
              ...
              <div class="flex items-center">
                ...
                <span class="font-medium mr-2">{{ $t('staking.contact') }}: </span>
            ...
            <p class="text-sm mt-4 mb-3 font-bold">{{ $t('staking.validator_status') }}</p>
            <div class="card-list">
              <div class="flex items-center mb-2">
                <Icon icon="mdi-shield-account-outline" class="text-xl mr-1" />
                <span class="font-medium mr-2">{{ $t('staking.status') }}: </span><span>
                  {{ String(v.status).replace('BOND_STATUS_', '') }}
                </span>
              </div>
              <div class="flex items-center">
                <Icon icon="mdi-shield-alert-outline" class="text-xl mr-1" />
                <span class="font-medium mr-2">{{ $t('staking.jailed') }}: </span>
                <span> {{ v.jailed || '-' }} </span>
              </div>
            </div>
            <p class="text-sm mt-4 mb-3 font-bold">{{ $t('staking.liquid_staking') }}</p>
            <div class="card-list">
              <div class="flex items-center mb-2">
                <Icon icon="mdi-lock" class="text-xl mr-1" />
                <span class="font-medium mr-2">{{ $t('staking.validator_bond_share') }}: </span>
              ...
              <div class="flex items-center">
                ...
                <span class="font-medium mr-2">{{ $t('staking.liquid_staking_shares') }}: </span>
                <span>
        ...
        <div class="flex-1">
          <div class="flex flex-col mt-10 gap-4">
            <div class="flex">
              <div class="flex items-center justify-center rounded-lg w-10 h-10" style="border: 1px solid #666">
      ...
      <div class="text-sm px-4 mt-3 py-3 border-t border-gray-300 dark:border-gray-700">{{ v.description?.details }}</div>
    </div>
    ...
    <div class="mt-3 grid grid-cols-1 md:!grid-cols-3 gap-4">
      ...
      <div class="bg-base-200 rounded-lg shadow relative overflow-auto">
      ...
      <div class="bg-base-200 rounded-lg shadow overflow-x-auto">
    ...
    <div v-if="delegations.delegation_responses" class="mt-5 bg-base-200 shadow rounded-lg p-4 ">
    ...
    <div class="mt-5 bg-base-200 shadow rounded-lg p-4">
      ...
      <div class="rounded-lg overflow-auto">
        <table class="table validatore-table w-full">
          ...
          <tbody>
            <tr v-if="!txs.tx_responses || txs.tx_responses.length === 0">
              <td colspan="99" class="py-8 text-center text-sm text-gray-400">
                No responses
              </td>
            </tr>
    ...
    <div class="mt-5 bg-base-200 shadow rounded-lg p-4">
      <div class="text-lg mb-4 font-semibold">
        <div class="tabs tabs-boxed !flex bg-transparent">
          <p class="mr-10">Voting Power Events: </p>
          <a class="tab" :class="{ 'tab-active': selectedEventType === EventType.Delegate }"
            @click="loadPowerEvents(1, EventType.Delegate)">{{ $t('account.btn_delegate') }}</a>
          <a class="tab" :class="{ 'tab-active': selectedEventType === EventType.Unbond }"
